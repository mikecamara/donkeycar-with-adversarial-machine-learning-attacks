...
class CreateAttackedBatch(BaseCommand):
...
class CreateMSE(BaseCommand):
    ...
    def plot_predictions(self, cfg, tub_paths, model_path, limit, model_type):
        ...
        error_mse_angles = mean_squared_error(user_angles, pilot_angles, squared=False)
        error_mse_throtlle = mean_squared_error(user_throttles, pilot_throttles, squared=False)
       
        # report error
        print("\n\nMSE steering angle")
        print(error_mse_angles)

        print("\nMSE throttle\n")
        print(error_mse_throtlle)
