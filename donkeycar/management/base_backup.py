...
# TODO instead of show prediction just copy normal images and thats all
...
class CreatedAttackedBatch(BaseCommand):
    ...
    def plot_predictions(self, cfg, tub_paths, model_path, limit, model_type):
        ...
        # tar = tarfile.open(os.path.expanduser('/Users/mikecamara/DonkeyCarProjects/donkeycar/donkeycar/tests/tub/tubattacked.tar.gz'))

        # tub_parent = os.path.join(base_path, 'data_ccmd/')
        # new_tub_parent = os.path.join(base_path, 'data_ccmd_flipped/')
        # tar.extractall(tub_parent)

        # tub_path = os.path.join(tub_parent, base_path)
        # tub1 = Tub(tub_path)
        tub1 = tub
        ...
        for key, record in enumerate(tub1):
            ...
            print("im about to set a terrible atack")
            # plt.imshow(adv_img)
            # plt.show()
            # plt.savefig('stopadv2000.png')
            # print("done and dust")
